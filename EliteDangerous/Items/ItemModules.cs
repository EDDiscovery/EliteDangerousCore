...
#if VANITYADD
            var newvanitymodules = new Dictionary<string, ShipModule>   // DO NOT USE DIRECTLY - public is for checking only
            {


            };

            foreach(var nm in newvanitymodules)
            {
                if (!vanitymodules.ContainsKey(nm.Key))
                    vanitymodules.Add(nm.Key,nm.Value);
            }
