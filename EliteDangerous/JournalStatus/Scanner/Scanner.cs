...
namespace EliteDangerousCore
{
    ...
    public class EDJournalUIScanner
    {
        ...
        {
            // start up on this index in this history.  We pick up the TLU from the latest entry with a TLU
            // if entry is on an autogen one (TLU=0) then we would need to move back to find the TLU.
            public SingleStepper(HistoryList hlist, int index)
            {
                var last = hlist.EntryOrder()[index];
                lastjid = last.Journalid;
                cmdrid = last.journalEntry.CommanderId;

                while( index >= 0)
                {
                    if (TravelLogUnit.TryGet(hlist.EntryOrder()[index].journalEntry.TLUId, out TravelLogUnit tlu))      // if got a valid TLU
                    {
                        journalreader = new EDJournalReader(tlu);
                        break;
                    }
                    else
                        index--;
        ...
        public bool SingleStep(SingleStepper stepper)
        {
            ...
            if ( next != null )
            {
                // some entries are autogenerated by EDD and thus are not in the TLU.
                // if we have a TLUID, get the TLU associated with the entry, and see if its changed

                // if changed TLU, move to next TLU
                if (next.TLUId > 0 && TravelLogUnit.TryGet(next.TLUId, out TravelLogUnit nexttlu) && nexttlu != stepper.tlu)
                {
