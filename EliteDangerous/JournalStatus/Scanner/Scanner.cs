    ...
    public class EDJournalUIScanner
    {
        ...
        private List<Events> GatherEvents(Func<bool> stopRequested)     
        {
            ...
            for (int i = 0; i < journalwatchers.Count; i++)
            {
                ...
                var evret = mw.ScanForNewEntries();             // return tuple of list of journal events and ui events
                ...
                var uiret = sw.Scan();                          // return tuple of list of journal events and ui events generated by scanner

                foreach (var ev in uiret.Item1.EmptyIfNull())
                {
                    events.Add(new Events() { je = ev, sr = sw });
                }

                foreach (var ev in uiret.Item2.EmptyIfNull())
                {
