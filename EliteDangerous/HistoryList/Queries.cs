...
namespace EliteDangerousCore
{
    public class HistoryListQueries       // search an HE list for items using the quries
    {
        ...
        public List<Query> Searches = new List<Query>()
            {
                new Query("Planet inside inner ring","(IsPlanet IsTrue And Parent.HasRings IsTrue And IsOrbitingBaryCentre IsFalse And nSemiMajorAxis <= Parent.RingsInnerm And Parent.Level >= 1) Or (IsPlanet IsTrue And IsOrbitingBaryCentre IsTrue And Parent.HasRings IsTrue And Barycentre_SemiMajorAxis <= Parent.RingsInnerm And Parent.Level >= 1)", QueryType.BuiltIn ),
                new Query("Planet inside rings","(IsPlanet IsTrue And Parent.HasRings IsTrue And IsOrbitingBaryCentre IsFalse And nSemiMajorAxis <= Parent.RingsOuterm And Parent.Level >= 1) Or (IsPlanet IsTrue And IsOrbitingBaryCentre IsTrue And Parent.HasRings IsTrue And Barycentre_SemiMajorAxis <= Parent.RingsOuterm And Parent.Level >= 1)", QueryType.BuiltIn ),
                new Query("Planet between inner and outer ring","(IsPlanet IsTrue And Parent.HasRings IsTrue And IsOrbitingBaryCentre IsFalse And nSemiMajorAxis >= Parent.RingsInnerm And nSemiMajorAxis <= Parent.RingsOuterm And Parent.Level >= 1) Or (IsPlanet IsTrue And IsOrbitingBaryCentre IsTrue And Parent.HasRings IsTrue And Barycentre_SemiMajorAxis >= Parent.RingsInnerm And Barycentre_SemiMajorAxis <= Parent.RingsOuterm And Parent.Level >= 1)", QueryType.BuiltIn ),
                new Query("Planet between rings 1 and 2","(IsPlanet IsTrue And Parent.HasRings IsTrue And IsOrbitingBaryCentre IsFalse And nSemiMajorAxis >= Parent.Rings[1]_OuterRad And nSemiMajorAxis <= Parent.Rings[2]_InnerRad And Parent.Level >= 1) Or (IsPlanet IsTrue And IsOrbitingBaryCentre IsTrue And Parent.HasRings IsTrue And Barycentre_SemiMajorAxis >= Parent.Rings[1]_OuterRad And Barycentre_SemiMajorAxis <= Parent.Rings[2]_InnerRad And Parent.Level >= 1)", QueryType.BuiltIn ),
                new Query("Planet between rings 2 and 3","(IsPlanet IsTrue And Parent.HasRings IsTrue And IsOrbitingBaryCentre IsFalse And nSemiMajorAxis >= Parent.Rings[2]_OuterRad And nSemiMajorAxis <= Parent.Rings[3]_InnerRad And Parent.Level >= 1) Or (IsPlanet IsTrue And IsOrbitingBaryCentre IsTrue And Parent.HasRings IsTrue And Barycentre_SemiMajorAxis >= Parent.Rings[2]_OuterRad And Barycentre_SemiMajorAxis <= Parent.Rings[3]_InnerRad And Parent.Level >= 1)", QueryType.BuiltIn ),
