...
namespace EliteDangerousCore.GMO
{
    public class GalacticMapping
    {
        ...
        public GalacticMapObject[] VisibleMapObjects { get { 
                lock (galobjects)
                return galobjects.Where(x => x.GalMapTypes[0] != null && x.GalMapTypes[0].VisibleType != null && x.StarSystem != null).ToArray(); 
            } }
        public GalacticMapObject[] AllObjects { get { lock (galobjects) { return galobjects.ToArray(); } } }

        public bool Loaded { get { return galobjects.Count > 0; } }
        ...
        { 
            ...
            lock (galobjects)
            {
                return csv.Read(t, (r, rw) =>
                {
                    ...
                    {
                        var previousstored = galobjects.Find(x => Math.Abs(x.Points[0].X - pos.X) < 0.25f &&
                    ...
                    lock( galobjects) 
                    {
                        foreach (JObject jo in galobjects)
                        {
                            ...
                            if (newgmo.Points.Count > 0) // need some sort of position
                            {
                                ...
                                var previousstored = this.galobjects.Find(x => Math.Abs(x.Points[0].X - newgmo.Points[0].X) < 0.25f &&
                                ...
                                if (previousstored != null && newgmo.Points.Count == 1)
                                {
                                    if ((newgmo.DescriptiveNames[0] == "Great Annihilator" || newgmo.DescriptiveNames[0] == "Galactic Centre")) // these take precedence
                                    {
                                        ...
                                        this.galobjects.Remove(previousstored);
                                        this.galobjects.Add(newgmo);
        ...
        public List<GalacticMapObject> FindDescriptiveNames(string descripivename, bool wildcard, bool contains)
        {
            ...
            if (descripivename.HasChars())
            {
                lock (galobjects)
                {
                    foreach (GalacticMapObject gmo in galobjects)
                    {
        ...
        private List<GalacticMapObject> galobjects = new List<GalacticMapObject>();
    }
}
