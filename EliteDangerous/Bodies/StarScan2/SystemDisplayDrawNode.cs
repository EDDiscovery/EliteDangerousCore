...
namespace EliteDangerousCore.StarScan2
{
    public partial class SystemDisplay
    {
                ...
                )
        {
            ...
            {
                // its a planet or star. Never a PlanetaryRing since screened out above this code

                var nodelabels = new string[2] { "", "" };

                nodelabels[0] = presentationname;

                bool toplevelstar = bn.BodyType == BodyDefinitions.BodyType.Star && bn.IsTopLevel(bn.BodyType);

                // set all of these to empty - scan will fill them in

                string imageoverlaytext = "";
                bool valuable = false, isdiscovered = false, terraformable = false, volcanism = false, mapped = false, previouslymapped = false, materialsicon = false, 
                                        hasatmosphere = false, landable = false;
                int ringcount = 0;
                Image nodeimage = BodyDefinitions.GetImageNotScanned();

                StringBuilder tooltip = new StringBuilder(1024);

                // if we have a scan
                if (bn.Scan != null && (!bn.Scan.IsWebSourced || ShowWebBodies))
                {
                    ...
                    if (sc.IsStar)                                      
                    {
                        ...
                        {
                            ringcount = sc.HasRingsOrBelts ? sc.Rings.Count() : 0;      // non stellar stars show rings
                        }
                    }
                    else
                    {
                        ...
                        mapped = sc.Mapped;
                        previouslymapped = sc.IsPreviouslyMapped;
                        materialsicon = sc.HasMaterials && !ShowMaterials;
                        ringcount = sc.HasRingsOrBelts ? sc.Rings.Count() : 0;
                        hasatmosphere = (sc.WaterGiant || !sc.GasWorld) && (sc.HasAtmosphericComposition || sc.HasAtmosphere);
                        landable = sc.IsLandable;

                        if (ShowPlanetClasses)
                            imageoverlaytext = Planets.PlanetAbv(sc.PlanetTypeID);
                ...
                int numiconoverlays = ShowOverlays ? (terraformable ? 1 : 0) + (volcanism ? 1 : 0) +
                                    (valuable ? 1 : 0) + (mapped ? 1 : 0) + (isdiscovered ? 1 : 0) + (previouslymapped ? 1 : 0) +
                                    (bn.Signals != null ? 1 : 0) + (bn.Organics != null ? 1 : 0) + (bn.CodexEntries != null ? 1 : 0) : 0;
                ...
                int imagewidthmultiplier16th = ringcount>0 ? 31 : materialsicon || landable ? 20 : 16;
                ...
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    if ( hasatmosphere) // show a atmosphere..
                    {
                    ...
                    if (numiconoverlays > 0)
                    {
                        ...
                        if (volcanism) //this renders below the terraformable icon if present
                        {
                    ...
                    if (imageoverlaytext.HasChars())
                    {
                        float ii = nodeimage.CentralImageIntensity();
                        Color text = ii > 0.3f ? Color.Black : Color.FromArgb(255, 200, 200, 200);

                        using (Font f = FontHandler.GetFont(Font.Name, size.Width / 5.0f))
                        {
                            using (Brush b = new SolidBrush(text))
                            {
                                g.DrawString(imageoverlaytext, f, b, new Rectangle(iconwidtharea, 0, bitmapwidth - iconwidtharea, bitmapheight), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                            }
                        }
                    }
                    ...
                    {
                        var matimages = CreateMaterialNodes(bn.Scan, historicmats, curmats, materialsize, rightclickmenumats);
