...
namespace EliteDangerousCore
{
    ...
    public class HistoryEntryStatus
    {
        public enum TravelStateType {
            ...
            Fighter,            // in fighter 
            Lander,             // in lander
            LanderLanded,       // in nomad on the surface
        ...
        public bool IsLandedInShipOrSRVOrLander { get
            {
            ...
            switch (je.EventTypeID)
            {
                case JournalTypeEnum.Location:
                    {
                        ...
                        TravelStateType t = (stationisfc && !jloc.Docked) ? TravelStateType.OnFootFleetCarrier :
                            prev.TravelState == TravelStateType.Lander || prev.TravelState == TravelStateType.LanderLanded ? prev.TravelState :
                ...
                case JournalTypeEnum.DockSRV:
                    ...
                    hes = new HistoryEntryStatus(prev)
                    {
                        TravelState = docksrv.IsLander ? TravelStateType.NormalSpace :
                                      prev.TravelState == TravelStateType.MulticrewSRV ? TravelStateType.MulticrewLanded : TravelStateType.Landed,
                    };
                    break;
                ...
                case JournalTypeEnum.Touchdown:
                    ...
                    if (td.PlayerControlled == true)        // can get this when not player controlled
                    {
                        hes = new HistoryEntryStatus(prev)
                        {
                            TravelState = prev.TravelState == TravelStateType.Lander ? TravelStateType.LanderLanded :
                                            prev.TravelState == TravelStateType.MulticrewNormalSpace ? TravelStateType.MulticrewLanded : TravelStateType.Landed,
                ...
                case JournalTypeEnum.Liftoff:
                    ...
                    if (loff.PlayerControlled == true)         // can get this when not player controlled
                    {
                        ...
                        {
                            TravelState = prev.TravelState == TravelStateType.LanderLanded ? TravelStateType.Lander :
                                        prev.TravelState == TravelStateType.MulticrewLanded ? TravelStateType.MulticrewNormalSpace : TravelStateType.NormalSpace,
                        };
                    }
                    else
                        hes = prev;
