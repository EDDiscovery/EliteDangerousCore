...
namespace EliteDangerousCore
{
    ...
    public class HistoryEntryStatus
    {
        public enum TravelStateType
        {
            ...
            Fighter,            // in fighter
            Lander,             // in lander
            LanderLanded,       // in lander on surface
        ...
        public VehicleFDName ShipTypeFD => CurrentShip?.ShipFD;         // may be null
            ...
            switch (je.EventTypeID)
            {
                ...
                case JournalTypeEnum.Location:
                    {
                        ...
                        if (!weareonafleetcarrieronfoot && !weareonaplanterarybase)
                        {
                            ...
                            TravelStateType t =
                                           hes.TravelState == TravelStateType.Lander || hes.TravelState == TravelStateType.LanderLanded ? hes.TravelState :
                ...
                case JournalTypeEnum.Touchdown:
                    ...
                    if (td.PlayerControlled == true)        // can get this when not player controlled
                    {
                        hes = new HistoryEntryStatus(this)
                        {
                            ...
                            TravelState = hes.TravelState == TravelStateType.Lander ? TravelStateType.LanderLanded :
                ...
                case JournalTypeEnum.Liftoff:
                    ...
                    if (loff.PlayerControlled == true)         // can get this when not player controlled
                    {
                        ...
                        {
                            TravelState = hes.TravelState == TravelStateType.LanderLanded ? TravelStateType.Lander :
                ...
                case JournalTypeEnum.LoadGame:
                    ...
                    hes = new HistoryEntryStatus(this)
                    {
                        ...
                        TravelState = jlg.ShipFD.Type == VehicleFDName.VehicleType.Suit ? (TravelState) :
                                         jlg.ShipFD.Type == VehicleFDName.VehicleType.Taxi ? TravelStateType.TaxiNormalSpace :
                                             jlg.ShipFD.Type == VehicleFDName.VehicleType.SRV ? TravelStateType.SRV :
                                                    TravelState != TravelStateType.Unknown ? TravelState :
                                                        TravelStateType.Docked,
                        CurrentShip = jlg.ShipFD.Type == VehicleFDName.VehicleType.Ship ? jlg : CurrentShip,
                        LastTaxiDropship = null,
                        CurrentBoost = 1,
                        LastDockingGranted = null,
                    };
                ...
                case JournalTypeEnum.Embark:        // foot-> SRV/Ship/Nomad in multicrew or not.
                    {
                        var em = (JournalEmbark)je;
                        hes = new HistoryEntryStatus(this)
                        {
                            TravelState = em.IsLander ? TravelStateType.LanderLanded :
                ...
                case JournalTypeEnum.LaunchFighter:
                    {
                        var jlf = je as JournalLaunchFighter;
                        if (jlf.PlayerControlled)
                        {
                            hes = new HistoryEntryStatus(this)
                            {
                                TravelState = jlf.IsLander ? TravelStateType.Lander :
                                            hes.TravelState == TravelStateType.MulticrewNormalSpace ? TravelStateType.MulticrewFighter : TravelStateType.Fighter,
                            };
                        }
                        else
                            ignored = true;
                ...
                case JournalTypeEnum.Loadout:
                    ...
                    if (jloadout.ShipFD.Type == VehicleFDName.VehicleType.Ship)     // if ship, make a new entry
                    {
                        hes = new HistoryEntryStatus(this)
                        {
